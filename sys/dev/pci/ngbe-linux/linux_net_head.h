...
#define PCI_ROM(v,d,name,info,data) {v,d,0,0,0,0,data}
struct pci_dev
{
...
{
    struct io_buffer *iobuf = malloc(len, M_DEVBUF, M_DONTWAIT);
    return iobuf;
}
...
static DECLARE_TASK_QUEUE(tq_ngbe);

static int schedule_task(struct tq_struct *task)
{
    queue_task(task, &tq_ngbe);
    return 0;
}
...
#define ngbe_rx_checksum(...)
#define ngbe_check_options(...)
#define ngbe_tx_queue_mapping(_adapter, _skb) ((_adapter)->tx_ring[0])
#define ngbe_reset_interrupt_capability(...)
#define ngbe_set_sriov_capability(...) do {} while(0)
#define ngbe_reset_sriov_capability(...) do {} while(0)
#define ngbe_configure_msix(...) do {} while(0)
...
static irqreturn_t ngbe_intr(int irq, void *data);
