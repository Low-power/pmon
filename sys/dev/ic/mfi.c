...
mfi_init_ccb(struct mfi_softc *sc)
{
	...
	for (i = 0; i < sc->sc_max_cmds; i++) {
		...
		error = bus_dmamap_create(sc->sc_dmat,
		    MAXPHYS, sc->sc_max_sgl, MAXPHYS, 0, /* wan: #define MAXPHYS (32 * 1024) // max raw I/O transfer size */
...
mfi_allocmem(struct mfi_softc *sc, size_t size)
{
	...
	if (mm == NULL)
		return (NULL);
	bzero(mm, sizeof(struct mfi_mem));//wan+ Fix
	...
	//fix up for mfi_intr()
	{
		uint32_t a[20];
		bzero(a, 20);
		((struct mfi_prod_cons *)(mm->am_kva))->mpc_producer = 0;
		((struct mfi_prod_cons *)(mm->am_kva))->mpc_consumer = 0;
//		((struct mfi_prod_cons *)(mm->am_kva))->mpc_reply_q = a;
	}
...
mfi_mgmt(struct mfi_softc *sc, uint32_t opc, uint32_t dir, uint32_t len,
    void *buf, uint8_t *mbox)//wan: edit cmd_block and post it
{
	struct mfi_ccb *ccb;
	int rv;
